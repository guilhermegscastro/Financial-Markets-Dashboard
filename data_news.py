...
def ft(theme):
    ...
    if theme != "deep_dive":
        ...
        for _, news in enumerate(editorial):
            ...
            if news.find("div", attrs={'class':"o-ads__outer"}) == None:
...
def fortune(theme):
    ...
    for i, news in enumerate(news_boxes):
        ...
        headline = news.find("h2", class_ = 'sc-3358ecfd-0 jORKgI card-title').a.get_text(strip=True)
        link = news.find("h2", class_ = 'sc-3358ecfd-0 jORKgI card-title').a['href']
...
def scraping_news():
    ...
    news.to_csv("csv_files/news.csv", index = False)
